        ...
        if (Instructor !== undefined && Instructor.length > 0) {
          ...
          return response.redirect("/myinfo/instructor");
            ...
            db.query('SELECT * FROM Student WHERE Email_Address = ? ',[email], function(error2, result){
              ...
              `,
              [result[0].Student_ID,result[0].Student_ID],(error3, results) => {
              ...
              [teacher,result[0][0].Course_ID, result[1][0].TIME_ID,result[2][0].DATE_ID],function(error2,ITD_result){
                ...
                [ITD_result[0].Teacher_ID,result[3][0].Student_ID,result[2][0].DATE_ID,result[1][0].TIME_ID,result[0][0].Course_ID,
                 ITD_result[0].Teacher_ID,result[3][0].Student_ID,result[2][0].DATE_ID,result[1][0].TIME_ID,result[0][0].Course_ID
                  ...
                  if(error3) throw error3;
                  db.query(`insert into refund_payment(imp_uid, reason, cancel_request_amount, Max_Count, Student_ID, Teacher_ID, TIME_ID, DATE_ID, Course_ID, Create_at) value(?,?,?,?,?,?,?,?,?,?)`,
                  [results[0][0].imp_uid, reason, results[1][0].Decrease_Count, results[1][0].Max_Count, result[3][0].Student_ID, ITD_result[0].Teacher_ID,result[1][0].TIME_ID,result[2][0].DATE_ID,result[0][0].Course_ID,date_now],function(error4,result){
                    if(error4) throw error4;
    ...
    if (Name && Phone_Number) { //필수정보
        ...
        `, [request.session.email,request.session.mobile], function(error1, results) { // DB에 같은 이름의 회원아이디가 있는지 확인
            ...
            if (results.length > 0 && request.session.email_mypage_check === 1) {     // DB에 같은 이름의 회원아이디가 있는지 확인
              db.query(`update Student set Name = ?, Address = ?, Phone_Number =? where Email_Address = ?`,[Name, Address, Phone_Number, request.session.email],(error2,result)=>{
                if(error2) throw error2;
              ...
              db.query(`update Student set Name = ?, Address = ?, Phone_Number = ? where Email_Address = ?;
                        update Instructor set Name = ?, Address = ?, Phone_Number =?,Description = ?,School =?;
              `,[Name,Address,Phone_Number,request.session.email,Name,Address,Phone_Number,self_introduce,school],(error2,result)=>{
                if(error2) throw error2;
                if (request.file !== undefined) {
                db.query(
                  `UPDATE Instructor SET Image=? WHERE Email_Address=?`,
                  [request.file.path, request.session.email],
                  function (error, Reviews) {}
                );
                }
          ...
          db.query(
            ...
            (error2, results) => {
              ...
              if (Number(post.Max) > Number(post.Increase)) {
                db.query(
                  //강의완료 업데이트
                  `UPDATE Section SET Increase_Count = ?, Decrease_Count = ? where Teacher_ID = ? and Course_ID = ? and DATE_ID = ? and TIME_ID = ?;
                  ...
                  [
                    ...
                    sanitizeHtml(Reviews[0].Teacher_ID),
                    sanitizeHtml(post.course),
                    sanitizeHtml(post.date),
                    sanitizeHtml(post.time),
                  ],
              ...
              } else if (sanitizeHtml(Number(post.Max)) === sanitizeHtml(Number(post.Increase))){
                db.query(
                  //강의삭제
                  ...
                  function (error2, result) {
                    ...
                    db.query(
                      ...
                      [0, sanitizeHtml(post.Teacher_ID), sanitizeHtml(post.course), sanitizeHtml(post.date), sanitizeHtml(post.time)],
                      function (error2, result) {}
                    );
                  }
                );
