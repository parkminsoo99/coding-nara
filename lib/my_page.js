                ...
                function (error2, result) {
                  db.query(
                    `select * from Section as S left join Instructor_Time_Date as ITD on S.Course_ID = ITD.Course_ID and S.TIME_ID = ITD.TIME_ID and S.DATE_ID = ITD.DATE_ID and ITD.Teacher_ID = S.Teacher_ID left join Course as C on S.Course_ID = C.Course_ID left join Instructor_TIME as IT on S.TIME_ID = IT.TIME_ID left join  Instructor_DATE as ID on S.DATE_ID = ID.DATE_ID where Student_ID = ?;
                          select * from Payment as P left join Instructor_Time_Date as ITD on P.Course_ID = ITD.Course_ID and P.TIME_ID = ITD.TIME_ID and P.DATE_ID = ITD.DATE_ID and ITD.Teacher_ID = P.Teacher_ID left join Course as C on P.Course_ID = C.Course_ID left join Instructor_TIME as IT on P.TIME_ID = IT.TIME_ID left join  Instructor_DATE as ID on P.DATE_ID = ID.DATE_ID where Student_ID = ?;
                `,
                    [result[0].Student_ID, result[0].Student_ID],
                    (error2, results) => {
                      if (error2) throw error2;
                      console.log("results", results[0]);
                      var lecture_list = template.lecture_list(results[0]);
...
router.post("/upload", upload.single("image"), (req, res, next) => {
  ...
  if (!tokens.verify(req.session.secret, token)) {
    ...
    db.query(`UPDATE Instructor SET Image=? WHERE Email_Address=?`,[req.file.path , email], function (error, Reviews) {
      console.log(email);
    });
  }
...
router.post("/validate_Mypage_Email", (request, response) => {
  ...
  if (!tokens.verify(request.session.secret, token)) {
    ...
    if (request.body.hideCK) {
      console.log("true");
      db.query(
        `select * from Student where Name = ? and Email_Address = ?`,
...
router.get("/refund_page/", (request, response) => {
  ...
  db.query(
    `select Course_ID from Course where Subject = ?;
            ...
            select Student_ID from Student where Email_Address = ?;
            `,
    [subject, time, day, request.session.email],
    function (error1, result) {
      ...
      db.query(
        ...
        [
          teacher,
          result[0][0].Course_ID,
          result[1][0].TIME_ID,
          result[2][0].DATE_ID,
        ],
        function (error2, ITD_result) {
          if (error2) throw error2;
          console.log("ITD", ITD_result);
          db.query(
            `select * from refund_payment where Course_ID =? and TIME_ID = ? and DATE_ID = ? and  Student_ID =? and Teacher_ID = ?;`,
            [
              result[0][0].Course_ID,
              result[1][0].TIME_ID,
              result[2][0].DATE_ID,
              result[3][0].Student_ID,
              ITD_result[0].Teacher_ID,
            ],
            function (error3, back_result) {
              if (error3) throw error3;
              if (back_result.length > 0) {
                response.send(`<script type="text/javascript">alert("이미 환불신청이 완료된 강좌입니다."); 
                        document.location.href="/myinfo";</script>`);
                ...
                db.query(
                  `select * from Section where Course_ID =? and TIME_ID = ? and DATE_ID = ? and  Student_ID =? and Teacher_ID = ?;`,
                  [
                    result[0][0].Course_ID,
                    result[1][0].TIME_ID,
                    result[2][0].DATE_ID,
      ...
      function (error1, result) {
        ...
        db.query(
          ...
          function (error2, ITD_result) {
            ...
            console.log(ITD_result);
            db.query(
              `select imp_uid from Payment where Teacher_ID =? and Student_ID =? and DATE_ID =? and TIME_ID =? and Course_ID = ?;
                          select Decrease_Count, Max_Count from Section where Teacher_ID =? and Student_ID =? and DATE_ID =? and TIME_ID =? and Course_ID = ?;
                `,
              [
                ITD_result[0].Teacher_ID,
              ...
              function (error3, results) {
                ...
                console.log(results);
                db.query(
                  `insert into refund_payment(imp_uid, reason, cancel_request_amount, Max_Count, Student_ID, Teacher_ID, TIME_ID, DATE_ID, Course_ID) value(?,?,?,?,?,?,?,?,?)`,
                  [
                    results[0][0].imp_uid,
                    reason,
                    results[1][0].Decrease_Count,
                    results[1][0].Max_Count,
                    result[3][0].Student_ID,
                    ITD_result[0].Teacher_ID,
                    result[1][0].TIME_ID,
                    result[2][0].DATE_ID,
...
router.post("/update_mypage", function (request, response) {
  ...
  if (!tokens.verify(request.session.secret, token)) {
    ...
    var Address_leftover = sanitizeHtml(request.body.member_leftover);
    var Address = Address_addr + " " + Address_leftover;
    if (Name && Phone_Number && Address_post && Address_addr) {
      //필수정보
      db.query(
        `SELECT * FROM Student WHERE Email_Address = ? or Phone_Number =?;
        `,
        [request.session.email, request.session.mobile],
        function (error1, results) {
          ...
          if (results.length > 0) {
            // DB에 같은 이름의 회원아이디가 있는지 확인
            db.query(
              `update Student set Name = ?, Address = ?, Phone_Number =?`,
...
=======
      db.query(
        `select * from Cart WHERE Teacher_ID = ? AND Course_ID = ? AND TIME_ID = ? AND DATE_ID = ?`,
        [post.Teacher_ID, post.course, post.time, post.date],
        (error1, cart_result) => {
          if (error1) throw error1;
          if (cart_result.length > 0) {
            db.query(
              `delete from Cart WHERE Teacher_ID = ? AND Course_ID = ? AND TIME_ID = ? AND DATE_ID = ?`,
              [post.Teacher_ID, post.course, post.time, post.date],
              (error3, cart_result) => {
                if (error3) throw error3;
              }
        ...
        function (error2, result) {
          ...
          db.query(
            ...
            (error2, results) => {
              ...
              if (Number(post.Max) > Number(post.Increase)) {
                console.log(123);
                db.query(
                  //강의완료 업데이트
              ...
              } else if (Number(post.Max) === Number(post.Increase)) {
                db.query(
                  //강의삭제
