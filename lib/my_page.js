...
router.get("/instructor", function (request, response) {
  ...
  if (!tokens.verify(request.session.secret, token)) {
    ...
    var email = request.session.email;
    db.query(`SELECT * FROM Instructor WHERE Instructor.Email_Address = ?;`,[email], function (error, Reviews) {
      console.log(Reviews);
        ...
        function (error2, result) {
          ...
          db.query(
            ...
            (error2, results) => {
              ...
              db.query( //강의완료 업데이트
                "UPDATE Section SET Teacher_ID = ?, Course_ID = ?, DATE_ID = ?, TIME_ID = ? ,Increase_Count = ?, Decrease_Count = ? ",[post.Teacher_ID, post.course, post.date, post.time, Number(post.Increase)+1, Number(post.Decrease)-1],
                function (error2, result) {
                  if (error2) throw error2;
                })
              }
              ...
              {
                db.query( //강의삭제
                  "DELETE FROM Section WHERE Teacher_ID = ? AND Course_ID = ? AND DATE_ID = ? AND TIME_ID = ?",[post.Teacher_ID, post.course, post.date, post.time],
                  function (error2, result) {
                    if (error2) throw error2;
                    db.query( //강의삭제
                      "UPDATE Instructor_Time_Date SET Course_Active = ?, Teacher_ID = ?, Course_ID = ?, DATE_ID = ?, TIME_ID = ?",[0,post.Teacher_ID, post.course, post.date, post.time],
                      function (error2, result) {
                      })
                  })
