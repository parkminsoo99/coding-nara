...
router.get('/sub_result', (req, res) => {
                    ...
                    */
                    ...
                    if(rsp.status == 'paid' && rsp.paid_amount == 100){
                        if (rsp.success) {
                            ...
                            alert(msg);
                            window.location.href = 'http://localhost:3000/enroll/payment/complete?point='+ discount_price + "&course_id=" + ${Course_Id} + "&time_id=" + ${Time_Id} + "&date_id=" + ${Date_Id} + "&teacher_id=" + ${Teacher_Id};
                            } 
                            else {
                                var msg = '결제에 실패하였습니다.';
 ...
 router.get('/payment/complete/',(req,res) => {
    ...
    db.query(
        ...
        insert into Section(Student_ID, Teacher_ID, Course_ID, DATE_ID, TIME_ID) values(?,?,?,?,?);
        `,[queryData.point,req.session.email
        , queryData.teacher_id, queryData.time_id, queryData.date_id, queryData.course_id
        , req.session.Student_Id, queryData.teacher_id , queryData.course_id, queryData.date_id, queryData.time_id],
