...
router.post('/save_price',(req,res) => {
    ...
    db.query(`select Course_ID from Course where Subject = ?;
              ...
              `,[req.body.Subject,req.body.Time,req.body.Day,req.session.email,req.body.Subject],function(error1,result)
              {
                if(error1) throw error1;
                console.log(result);
                ...
                [req.body.Instructor_name,result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID],function(error2,ITD_result){
                    ...
                    [result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID,result[3][count].Student_ID, ITD_result[0].Teacher_ID],function(error3,results){
                        ...
                        console.log('results123',results);
                            ...
                            [((req.body.Count * result[4][count].Price) - results[0].Use_Point),results[0].Student_ID,results[0].Teacher_ID, results[0].TIME_ID,results[0].DATE_ID,results[0].Course_ID],
                            function(error4,insert){
                                if(error4) throw error4;
                                console.log('results12342134213423');
                            })
                    })
                })   
            })
    ...
    db.query(`select Course_ID from Course where Subject = ?;
              ...
              `,[req.body.Subject,req.body.Time,req.body.Day,req.session.email],function(error1,result)
              {
                ...
                [req.body.Instructor_name,result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID],function(error2,ITD_result){
                    ...
                    [result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID,result[3][count].Student_ID, ITD_result[0].Teacher_ID],function(error3,results){
                        if(error3) throw error3;
                        console.log('results111222333',results);
                            ...
                            results[0].Use_Point, results[0].Student_ID,
                            req.body.Imp_Uid, req.body.Merchant_Uid, results[0].Student_ID, results[0].Teacher_ID, results[0].DATE_ID, results[0].TIME_ID, results[0].Course_ID, date_now, req.body.Count, results[0].Use_Point, results[0].Payment_Price],
                            function(error4,insert){
                                if(error4) throw error4;
                            })
                    })
                ...
                [req.body.Instructor_name,result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID],function(error2,ITD_result){
                    ...
                    [result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID,result[3][count].Student_ID, ITD_result[0].Teacher_ID],function(error3,results){
                        ...
                        console.log('results111113',results);
                            ...
                            [req.body.Point,results[0].Student_ID,results[0].Teacher_ID, results[0].TIME_ID,results[0].DATE_ID,results[0].Course_ID],
                            function(error4,insert){
                                if(error4) throw error4;
                                console.log('??sdfsafsadfsaddf????????')                           
                            })
                    })
                })   
            })
...
router.post('/delete_cart',(req,res) => {
    ...
    `,[req.body.Subject,req.body.Time,req.body.Day,req.session.email],function(error1,result)
    {
      ...
      [req.body.Instructor_name,result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID],function(error2,ITD_result){
          ...
          [result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID,result[3][count].Student_ID, ITD_result[0].Teacher_ID],function(error3,results){
              if(error3) throw error3;
              console.log('results1114324',results);
                  db.query(`update Cart set Use_Point = ?, payment_Price = ? where Student_ID =? and Teacher_ID = ? and TIME_ID = ? and DATE_ID = ? and Course_ID = ?;`,
                  [0,0, results[0].Student_ID, results[0].Teacher_ID, results[0].TIME_ID,results[0].DATE_ID,results[0].Course_ID],
                  function(error4,insert){
                      if(error4) throw error4;
                  })
          })
...
router.post('/get_result_price_point',(req,res)=>{
    ...
    {
      if(error1) throw error1;
      console.log('1231234',result);
      console.log('time123',result[3][count].Phone_Number)
      db.query(`select Teacher_ID from Instructor_TIME_DATE where Name = ? and Course_ID = ? and TIME_ID = ? and DATE_ID = ?;`,
      [req.body.Instructor_name,result[0][count].Course_ID, result[1][count].TIME_ID,result[2][count].DATE_ID],function(error2,ITD_result){
          if(error2) throw(error2);
          console.log('ITD',ITD_result)
    ...
    if (!tokens.verify(req.session.secret, token)) {
      throw new Error('invalid token!')
        ...
        db.query(
            ...
            insert into Payment(imp_uid, merchant_uid, Teacher_ID, DATE_ID, TIME_ID, Course_ID, Discount_price, Final_price) value(?,?,?,?,?,?,?,?,?);
            ...
            , queryData.imp_uid, queryData.merchant_uid,req.session.Student_Id, queryData.teacher_id, queryData.date_id, queryData.time_id, queryData.course_id, queryData.original_price, queryData.point, queryData.final_price
        ],
