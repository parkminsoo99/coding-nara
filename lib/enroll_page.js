...
router.get('/payment/complete',(req,res) => {
    ...
    db.query('update Student set Point = Point - ? where Email_Address = ?',[queryData.point,req.session.email],function(error, result){
     if(error) throw error;
     res.redirect('/');
    });
 })
 router.get('/:CourseId/:DateId/:TIMEId', (req,res) => {
         ...
         if(rsp.status == 'paid' && rsp.paid_amount == 100){
           if (rsp.success) {
             ...
             discount_price = Number(discount_price.replaceAll(',', ''));
               ...
               alert(msg);
               window.location.href = "http://localhost:3000/enroll/payment/complete?point=" + discount_price;
           } else {
               var msg = '결제에 실패하였습니다.';
