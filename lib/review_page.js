...
router.get("/", function (request, response) {
  ...
  if (!tokens.verify(request.session.secret, token)) {
    ...
    db.query(
      `SELECT * FROM Review LEFT JOIN Student ON Review.Student_ID = Student.Student_ID LEFT JOIN Course ON Review.Order_subject = Course.Course_ID limit 0, 10;
    ...
    if (!authCheck.isOwner(request, response)) {
      ...
      db.query(
        ...
        function (error, Reviews) {
          ...
          console.log("reviews.", request.session.Student_Id);
          console.log(Number(Reviews[0].Point));

          if (error) {
            throw error;
          }
          if (Reviews[0].Recommand_Count === 0) {
            //포인트추가
            console.log(Reviews[0].Point);
            let point;
...
router.post("/increase_recommand", function (request, response) {
  ...
  if (!tokens.verify(request.session.secret, token)) {
    ...
    if (!authCheck.isOwner(request, response)) {
      ...
      db.query(
        `update review set Recommand_Count = ? where Review_ID =? `,
        [request.body.New_num, request.body.Order],
...
router.get("/:pageId", function (request, response) {
  ...
  if (!tokens.verify(request.session.secret, token)) {
    ...
    db.query(
      `select * from Review LEFT JOIN Student ON Review.Student_ID = Student.Student_ID LEFT JOIN Course ON Review.Order_subject = Course.Course_ID limit ?,10;
              select count(*) as c from Review;
    `,
      [number],
