module.exports = {
  HTML:function(title, list, body, control, authStatusUI){
    ...
    while(i < topics.length){
      list = list + `
      <li>
        <a href="/enroll/${topics[i].Teacher_ID}/${topics[i].Course_ID}/${topics[i].DATE_ID}/${topics[i].TIME_ID}"> 
            <div>
            <p>Name :  ${topics[i].Name} </p>
            </div>
            <div align="center"> Course_ID : ${topics[i].Subject}</div>
            <div align="right"><p>Time_ID : ${topics[i].Available_Start_Time} Date_ID : ${topics[i].Available_DATE} </p></div>
          </a>
       </li>
       `;
      i = i + 1;
