...
module.exports = {
  list:function(topics){
    ...
    while(i < topics.length){
      list = list + `
      <!—BOX-1———————>
    <div class="testimonial-box col-3">
      <!———————top————————————>
      <div class="box-top">
        <!————————————profile——>
        <div class="profile">
          ...
          <!——name-and-username——>
          <div class="name-user">
            <strong>${topics[i].Order_subject}</strong>
            <span>${topics[i].Name[0]}&#9675;${topics[i].Name[2]}</span>
          </div>
        </div>
        <!——reviews———>
        <div style="display:flex;justify-content : center;flex-direction: column;">
          <button id="p_button${topics[i].Review_ID}" style="border: none; background-color:transparent; "onclick="javascript:review_list.recommand_change_num(${topics[i].Review_ID});"><i class="bi bi-heart-fill fs-3 text-primary"></i></button>
     	  <center>
          	<span name="p_num${topics[i].Review_ID}" id="p_num${topics[i].Review_ID}" class="p_num" >${topics[i].Recommand_Count}</span>
	  </center>
          <input type="hidden" id="number${topics[i].Review_ID}" name="number${topics[i].Review_ID}" value="${topics[i].Review_ID}"/>
        </div>
      </div>
      <!———Comments————————————————————>
      <div class="client-comment">
        <p>${topics[i].Description}</p>
        <p>${topics[i].Create_at[0]}${topics[i].Create_at[1]}${topics[i].Create_at[2]}${topics[i].Create_at[3]}${topics[i].Create_at[4]}${topics[i].Create_at[5]}${topics[i].Create_at[6]}${topics[i].Create_at[7]}${topics[i].Create_at[8]}${topics[i].Create_at[9]}${topics[i].Create_at[10]}${topics[i].Create_at[11]}${topics[i].Create_at[12]}${topics[i].Create_at[13]}${topics[i].Create_at[14]}${topics[i].Create_at[15]}</p>
        </div>
      </div>
     `
                    ;
  ...
  lecture_list:function(lecture){
    ...
    while(i < lecture.length){
      list = list + `
                    ...
                    <td>
                      <a id="review_button${i}" onclick="javascript:mypage_list.review_button(${i})"><i class="bi bi-pencil fs-3 text-primary"></a></i>
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      id="modal${i}"
                      class="btn btn-dark"
                      onclick="javascript:basket.add_isrefund('${lecture[i].Subject}', '${lecture[i].Available_Start_Time}','${lecture[i].Available_DATE}', '${lecture[i].Name}',${i});">
                      추가 결제
                      </button>
                      <dialog id="dialog${i}" style="height: 10vh; width: 30vh">
                        <div align="center" style="margin-bottom : 10px;">
                          <span>강의 횟수 : <input type="text" id="add_count${i}" name="add_count${i}" min="1" max="99" size="3" maxlength="2"  onkeyup="javascript:basket.add_Point(${i});"></span>                    
                        </div>
                        <div style="display : flex; align-items: center; justify-content: center;">
                          <form method="dialog">
                            <button style="margin-right:5px" class="btn btn-secondary" id="add_enroll_button${i}" onclick="javascript:basket.add_payment('${lecture[i].Subject}', '${lecture[i].Available_Start_Time}','${lecture[i].Available_DATE}', '${lecture[i].Name}',${i});">결제하기</button>
                          </form>
                          <form method="dialog">
                            <button align="center" class="btn btn-secondary">닫기</button>
                          </form>
                        </div>
                      </dialog>
                    </td>
    ...
    while(i < lecture.length){
      list = list + `
                      <tr class="cart__list__detail">
                        ...
                        <td>
                          <img id="image${i}" src="" style="height:60px;width:60px;" alt="${lecture[i].Subject}" /> 
                        </td>
                        
                        <td>
                          <div class="information">
                            <span class="cart__list__smartstore" id="subject${i}">${lecture[i].Subject}</span>
                      ...
                      <script> 
                        Show_Cart_Course_info("${lecture[i].Subject}",${i}); 
                      </script>
                    `;
      i = i + 1;
    }
