module.exports = {
  HTML:function(title, list, body, control, authStatusUI){
    ...
    while(i < topics.length){
      list = list + `<li><a href="/page/${topics[i].id}">${topics[i].title}<div align="center">${topics[i].description}</div><div align="right">${topics[i].created}</div></a></li>`;
      i = i + 1;
    }
    list = list+'</ol>';
    return list;
    ...
    while(i < authors.length){
      ...
      tag += `<option value="${authors[i].Student_ID}"${selected}>${authors[i].userid}</option>`;
      i++;
    }
    return `
      <select name="author">
